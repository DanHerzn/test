  ...
  res.send(`
    ...
    <head>
      ...
      <style>
        ...
        .header { display: flex; justify-content: space-between; align-items: center; border-bottom: 2px solid #6A0DAD; padding-bottom: 1rem; margin-bottom: 1.5rem; }
        ...
        async function loadTable() {
          ...
          try {
            ...
            tbody.innerHTML = data.map(row => {
              const prob = row.purchase_probability ? parseFloat(row.purchase_probability) : null;
              const badgeClass = prob >= 0.75 ? 'badge-high' : 'badge-low';
              return \`
                <tr>
                  <td><strong>\${row.sku}</strong></td>
                  <td>\${row.name}</td>
                  <td>\${Number(row.current_stock).toLocaleString()} lb</td>
                  <td>\${row.predicted_demand_qty ? Number(row.predicted_demand_qty).toLocaleString() + ' lb' : 'Sin pronóstico'}</td>
                  <td>\${prob !== null ? \`<span class="badge \${badgeClass}">\${(prob * 100).toFixed(1)}%</span>\` : 'N/A'}</td>
